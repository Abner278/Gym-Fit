...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['google_credential'])) {
    ...
    if (isset($data['email']) && $data['aud'] == GOOGLE_CLIENT_ID) {
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            ...
            // Update profile image to ensure it's synced and fix any previous truncation issues
            mysqli_query($link, "UPDATE users SET profile_image = '$profile_image' WHERE id = " . $row['id']);
        } else {
